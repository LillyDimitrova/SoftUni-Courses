...
public class ControllerImpl implements Controller {
    ...
    public String reserve(int numberOfPeople) {
        Table table = tableRepository.getAllEntities().stream().filter(t -> (!t.isReservedTable()) && t.getSize() >= numberOfPeople)
                .findFirst()
                .orElse(null);
        if (table == null) {
    ...
    public String closedBill(int tableNumber) {
        double currentBill = tableRepository.byNumber(tableNumber).bill();
        totalMoney += currentBill;
        tableRepository.byNumber(tableNumber).clear();

        return String.format(OutputMessages.BILL, tableNumber, currentBill);
    }
